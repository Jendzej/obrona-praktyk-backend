...
from src.data_functions.status_and_roles_functions import StatusRoleFunctions
...
async def startup():
    for single_school_class in startup_school_class:
        if school_class.insert(single_school_class, True):
            logger.debug(f"Successfully added '{single_school_class}' to school classes!")
    for single_role in startup_roles:
        if status_role_function.insert_role(single_role):
            logger.debug(f"Successfully adedd `{single_role}` to roles!")
    for single_status in startup_payment_status:
        if status_role_function.insert_status(single_status):
            logger.debug(f"Successfully added '{single_status}' to payment statuses!")
